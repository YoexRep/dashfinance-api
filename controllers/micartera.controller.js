...
const getMiCartera = async (req, res) => {
  ...
  const response = await pool.query(
    "SELECT * FROM micartera2 where id_usuario = $1",
    [id]
  );
  res.json(response.rows);
};
...
const setMiCartera = async (req, res) => {
  ...
  try {
    ...
    const response = await pool.query(
      'INSERT INTO "public".micartera2 ( tipo, criptomoneda, cantidad, precio_compra, id_usuario, comision  ) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
